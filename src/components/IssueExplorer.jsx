...
export default function IssueExplorer(){
 ...
 return <section className="issue-explorer section" id="issue-explorer" aria-labelledby="explorer-heading">
   ...
   {path&&personaId&&!issueId&&<div className="explorer-step"><button type="button" className="explorer-back" onClick={()=>setPersonaId(null)}>← Back</button><h3>{path.issueStep.question}</h3><div className="explorer-options explorer-options--pills">{path.issueStep.options.map(o=><button key={o.id} type="button" className="explorer-pill" onClick={()=>setIssueId(o.id)}>{o.label}</button>)}</div></div>}
