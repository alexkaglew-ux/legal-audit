...
const WORLDS = [
  { id: "practice", number: "01", title: "The Practice", line: "Sports. Music. Television + Film.", href: "#practice" },
  ...
  return (
    <section className="world-nav" aria-label="Explore Alex's work">
      <div className="world-nav-stage">
        ...
        <div className="world-nav-copy" aria-live="polite"><span>{current.number} / 05</span><h2>{current.title}</h2><p>{current.line}</p><a href={current.href}>Enter →</a></div>
      </div>
      <div className="world-nav-list" role="tablist" aria-label="Areas to explore">
        {WORLDS.map((item, index) => (
          <button type="button" key={item.id} className={active === index ? "active" : ""} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => setActive(index)} role="tab" aria-selected={active === index}>
            <span>{item.number}</span><b>{item.title}</b><i>↗</i>
          </button>
        ))}
      </div>
    </section>
